...
The source directory is always **`<KIT ROOT>/.claude/`** — i.e. the `.claude` folder at the root of this kit (the same kit the user copied in; not a hard-coded repo path).
...
<target>/
  .claude/
    commands/
      ck/       <selected command files, e.g. ck/cook.md>
    ...
    rules/  ck-command-design.md  ck-agent-design.md  ck-skill-design.md
...
From `<KIT ROOT>/.claude/` → `<target>/.claude/`:
...
- `rules/ck-command-design.md`, `rules/ck-agent-design.md`, `rules/ck-skill-design.md`
...
For each skill selected in Step 2, copy `<KIT ROOT>/.claude/skills/<name>/` → `<target>/.claude/skills/<name>/` recursively, skipping any `node_modules/` subtrees. In Merge mode, skip skill directories that already exist at the destination.
...
| -------------------------------------- | -------------------------------------- | ------------- |
| SessionStart `startup\|resume\|clear\|compact` | `session_init.py`              | always        |
| SessionStart `startup\|resume\|clear\|compact` | `subagent_init.py`             | always        |
...
| Stop                                  | `session_end.py` (timeout 10)        | always        |
| SubagentStop                          | `session_end.py` (timeout 10)        | always        |

Each entry is its own matcher block: `{"matcher": "<matcher>", "hooks": [{"type": "command", "command": "python \"$CLAUDE_PROJECT_DIR/.claude/hooks/<script>.py\"", "timeout": <N>}]}` (omit `matcher` for SessionStart-less-specific / UserPromptSubmit / PreCompact / Stop / SubagentStop entries that apply to every source/tool).
...
Design-rule reference docs live in `.claude/rules/` (not auto-loaded — @-mention when creating agents/commands/skills):

| File | Covers |
|------|--------|
| `.claude/rules/ck-agent-design.md` | `.claude/agents/**` |
| `.claude/rules/ck-command-design.md` | `.claude/commands/**` |
| `.claude/rules/ck-skill-design.md` | `.claude/skills/**` |
```
